...
var GoL;
(function (GoL) {
    function Organism(color) {
        "use strict";
        var X = 300, Y = 300, cellSize = 12, width = Math.floor(X / cellSize), height = Math.floor(Y / cellSize), dead = 0, alive = 1, delay = 500, stopped = 0, running = 1, minimum = 2, maximum = 3, spawn = 3, state = stopped, interval = null, grid = matrix(height, width), cellColor;
        if (color) {
            cellColor = color;
        }
        else {
        ...
        function updateState() {
            ...
            */
            function calculateNeighbours(y, x) {
                ...
                for (var h = -1; h <= 1; h++) {
                    for (var w = -1; w <= 1; w++) {
                        if (grid[(height + (y + h)) % height][(width + (x + w)) % width] !== dead) {
                            total++;
                        }
                    }
                }
        ...
        function clickEvent(event) {
            ...
            */
            function getCursorPosition(event) {
                ...
                var cell = new GoL.Cell(Math.floor((y - 4) / cellSize), Math.floor((x - 2) / cellSize));
                return cell;
            }
        }
        /**
            ...
            if (gridCanvas.getContext) {
                ...
                for (var h = 0; h < height; h++) {
                    for (var w = 0; w < width; w++) {
                        ...
                        context.fillRect(w * cellSize + 1, h * cellSize + 1, cellSize - 1, cellSize - 1);
                    }
                }
            }
            else {
